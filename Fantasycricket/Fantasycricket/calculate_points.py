import sqlite3
#connecting to fantasy_cricket.db
...
def calculate_points(row):
    ...
    try:
        strike_rate = float(row[1]) / float(row[2])  # as strike rate = runs/balls faced
    ...
    # try block to catch exceptions
    try:
        economy = float(row[7]) / (float(row[5]) / 6) # economy = runs/over
    # except block to handle exceptions
    except:
        economy = 0
    Fielding = float(row[9]) + float(row[10]) + float(row[11])

    # 1 point for hitting a boundary and 2 runs
    # 2 points for over boundary
    # 10 points each for catch and wicket
    
    points += (fours + 2 * sixes + 10 * Fielding + twos + wickets)
    if score > 100:
        points += 10  # Additional 10 points for century
    elif score >= 50:
        points += 5  # Additional 5 points for half century
        
    if strike_rate > 1:  # Additional 4 points for strike rate>100
        points += 4
